...
public class HappyScoreMapper extends Mapper<LongWritable, Text, Text, DoubleWritable> {
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        if(author.equals("fotopaper") || author.equals("rocky_m") || author.equals("ethics")){


            Text body = new Text(obj.getString("body"));
            //i'm from, i live in,
            String bodyString = body.toString().toLowerCase();
            DoubleWritable score = getSentimentScore(bodyString);

            try {
                context.write(new Text(author), score);
